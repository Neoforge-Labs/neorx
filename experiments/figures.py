...
def fig1_pipeline() -> None:
    ...
    for label, x, color, stage_label in stages:
        ...
        ax.add_patch(box)
        ax.text(
            x, 2.7, label, ha="center", va="center",
...
def fig2_hiv_graph() -> None:
    ...
    confidences = {
        "POL": "C=0.990", "ENV": "C=0.955", "GAG": "C=0.887",
        "CCR5": "C=0.706", "CD4": "C=0.625", "CXCR4": "C=0.580",
        "RPOA": "INCON.", "RPSA": "INCON.",
    }

    classifications = {
        "POL": "PATHOGEN_DIRECT", "ENV": "PATHOGEN_DIRECT",
        "GAG": "PATHOGEN_DIRECT", "CCR5": "HOST_INVASION",
        "CD4": "HOST_IMMUNE", "CXCR4": "CORRELATIONAL",
        "RPOA": "INCONCLUSIVE", "RPSA": "INCONCLUSIVE",
    }
    ...
    for name, (x, y, color, ntype) in nodes.items():
        r = 0.55 if name != "HIV" else 0.7
        circle = plt.Circle((x, y), r, facecolor=color, edgecolor="#333", linewidth=1.5, zorder=2)
        ax.add_patch(circle)
        ax.text(
            x, y + 0.05, name, ha="center", va="center",
            fontsize=10 if name != "HIV" else 12, fontweight="bold", zorder=3,
        )

        if name in confidences:
            ax.text(x, y - r - 0.25, confidences[name], ha="center", va="top", fontsize=7, color="#555")
            ax.text(
                x, y - r - 0.55, classifications[name],
...
def _render_fig3(series: dict) -> None:
    ...
    ax = axes[0]
    bars1 = ax.bar(
        x - width / 2, corr_f1, width, label="Correlation-only",
        color=GREY, alpha=0.8, edgecolor="white",
    )
    bars2 = ax.bar(
        x + width / 2, neorx_f1, width, label="NeoRx",
    ...
    if grades and len(grades) == len(diseases):
        for bar, grade in zip(bars2, grades):
            if not grade:
                continue
            ax.text(
                bar.get_x() + bar.get_width() / 2, bar.get_height() + LABEL_OFFSET, grade,
    ...
    if neorx_f1:
        ...
        ax.text(
            len(diseases) - 0.7, mean_neorx + LABEL_OFFSET, f"Mean={mean_neorx:.3f}",
    ...
    if corr_f1:
        ...
        ax.text(
            len(diseases) - 0.7, mean_corr + LABEL_OFFSET, f"Mean={mean_corr:.3f}",
            fontsize=7, color=GREY,
        )

    if has_pr:
        ax = axes[1]
        ax.bar(x - width / 2, corr_p, width, label="Correlation-only", color=GREY, alpha=0.8, edgecolor="white")
        ax.bar(x + width / 2, neorx_p, width, label="NeoRx", color=ORANGE, alpha=0.9, edgecolor="white")
        ...
        ax = axes[2]
        ax.bar(x - width / 2, corr_r, width, label="Correlation-only", color=GREY, alpha=0.8, edgecolor="white")
        ax.bar(x + width / 2, neorx_r, width, label="NeoRx", color=TEAL, alpha=0.9, edgecolor="white")
...
def fig4_disease_context() -> None:
    ...
    ax.set_title("Disease-Context-Dependent Gene Classification", fontsize=13, fontweight="bold", pad=20)
