...
def identify_causal_targets(
    ...
    frame : frozenset[str] | None
        The population a dated run may evaluate -- gene symbols the pinned
        release associated with this disease on genetic evidence (see
        ``neorx.snapshots.frame.candidate_frame``). ``None`` means
        undated, unchanged behaviour: every eligible node is a candidate.
    ...
    return rank_causal_targets(
        evaluate_all_targets(graph, frame=frame),
        top_n=top_n,
...
def evaluate_all_targets(
    ...
    ``frame``, when given, restricts candidates to the population a dated
    run may evaluate (see ``identify_causal_targets``). Exclusions are
    logged rather than raised: Monarch and ChEMBL legitimately contribute
    genes OpenTargets did not, so refusing on an out-of-frame gene would
    fail every dated run for an entirely normal condition. The log line,
    with the disease name and the count, is what keeps that filtering
    honest -- a run that suddenly excludes far more than it used to has
    had something change upstream, and that must be visible.
    ...
    candidates, frame_excluded = _get_candidate_nodes(G, disease_node_id, frame=frame)
    if frame_excluded:
        logger.info(
            "Frame gate excluded %d candidate nodes for %r.",
            len(frame_excluded), graph.disease_name,
        )
