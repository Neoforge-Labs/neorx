...
- ``neorx.genmol.generate.generate()`` → list[str]
- ``neorx.molscreen.properties.calculate_properties()`` → MolecularProperties
- ``neorx.molscreen.accessibility.qed_score()`` / ``sa_score()``
- ``neorx.molscreen.filters.run_all_filters()`` → list[FilterResult]
- ``neorx.molscreen.similarity.find_similar_drugs()`` → novelty
- ``neorx.dockbot.protein_prep.prepare_protein()`` → ProteinInfo
- ``neorx.dockbot.ligand_prep.prepare_ligand_pdbqt()`` → (Mol, pdbqt)
- ``neorx.dockbot.docker.dock()`` → DockingResult
    ...
    *,
    ...
    try:
        ...
        if generate_report:
            ...
            try:
                from neorx.core.report import generate_report as gen_report
...
def _generate_for_target(
    ...
    try:
        ...
        from neorx.genmol.generate import generate
...
def _screen_candidates(
    ...
    if run_docking and target.pdb_ids:
        ...
        try:
            from neorx.dockbot.protein_prep import prepare_protein
...
def _get_mol_properties(smiles: str) -> dict[str, Any] | None:
    ...
    try:
        from neorx.molscreen.parser import validate_smiles
        if not validate_smiles(smiles):
            return None

        from neorx.molscreen.properties import calculate_properties
        from neorx.molscreen.accessibility import qed_score, sa_score
        from neorx.molscreen.filters import run_all_filters
...
def _run_docking(
    ...
    """
    try:
        from neorx.dockbot.ligand_prep import prepare_ligand_pdbqt
        from neorx.dockbot.docker import dock
...
def _detect_binding_site(
    ...
    3. Fall back to the geometric centre of the protein with a
       large search box.
    """
    from neorx.dockbot.models import BindingSite
