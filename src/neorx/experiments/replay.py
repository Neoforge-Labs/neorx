...
from neorx.experiments.registry import get_experiment, settle_or_refuse
...
def replay_experiment(run_id: str, *, runs_dir: Path | None = None) -> ReplayResult:
    ...
    # agreement between two sets of numbers whose inputs it cannot
    # identify. The two paths drifted apart once; sharing the call is what
    # keeps them together.
    settle_or_refuse(replay_rec, f"{defn.name}-replay", declared=defn.reads_snapshots)
